...
def findPrediction(row):
	...
	return functions.sign(myrules.findDecision(params))
...
def apply(df, config, header, dataset_features, validation_df = None, process_id = None):
	...
	pbar = tqdm(range(num_of_weak_classifier), desc='Adaboosting')
	root = 1
	for i in pbar:
		worksheet['Decision'] = worksheet['Weight'] * worksheet['Decision']

		file = f"outputs/rules/rules_{str(i)}.py"
		...
		moduleName = f"outputs/rules/rules_{str(i)}"
		fp, pathname, description = imp.find_module(moduleName)
		myrules = imp.load_module(moduleName, fp, pathname, description)
	...
	models = models[:best_epoch_idx+1]
	alphas = alphas[:best_epoch_idx+1]
...
def addEpochAlpha(epoch, alpha):
	file = "outputs/rules/alphas.py"
	content = f"   if epoch == {str(epoch)}" + ":\n"
	content += f"      return {str(alpha)}"
